...
def generate_html(command_list):
    ...
    for cog in command_list:
        ...
        for c in commands:
            containers += '<li class="command {}"><div class="command-name"><span style="display: inline-block;background: #{};padding: 5px;text-shadow: 2px 2px 2px rgba(0,0,0,0.5);">{}</span>&nbsp;&nbsp;{}</div><div class="command-use">{}</div><div class="command-description">{}</div></li>\n'.format(cog.lower(), c.get('color', '615d5d'), cog, make_name(c['name'], c['aliases']), make_parameters(c['name'], c['args']), c['comment'].replace('(Mod Only)', '<b>(Mod Only)</b>').replace('((You) Mod Only)', '<b>((You) Mod Only)</b>').replace('(NSFW channels Only)', '<b>(NSFW channels Only)</b>'))
