...
class GBF_Utility(commands.Cog):
    ...
    async def lucilius(self, ctx, page : int = 0):
        ...
        for description in queue:
            try:
                await ctx.author.send(embed=self.bot.buildEmbed(title="{} Dark Rapture (Hard) ▫️ Buncle Guide".format(self.bot.getEmote('lucilius')), description=description, footer="version 1.1", color=self.color))
                await ctx.message.add_reaction('✅') # white check mark
            except:
