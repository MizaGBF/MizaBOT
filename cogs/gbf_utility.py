...
class GBF_Utility(commands.Cog):
    ...
    async def mizatube(self, ctx, *, query : str = ""):
        ...
        Add -s to get only the first result"""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as r:
                if r.status != 200:
                    ...
                    out = []
                    while running:
                        # search VideoID then Label for title
                        # two VideoID back to back won't work so we check for it
                        # might break if youtube changes
                        a = s.find('ideoRenderer":{"videoId":"', cur) # from VideoID
                        if a == -1: break
                        a += len('ideoRenderer":{"videoId":"')
                        b = s.find('"', a) # to the closing "
                        if b == -1: break
                        code = s[a:b] # get code
                        cur = b # move our virtual cursor
                        c = s.find('ideoRenderer":{"videoId":"', cur) # search the next VideoID
                        a = s.find('"accessibility":{"accessibilityData":{"label":"', cur) # from label
                        if a == -1 or a > c: continue # if next VideoID is before, we skip
                        a += len('"accessibility":{"accessibilityData":{"label":"')
                        b = s.find('"}}', a) # to the closing "
                        if b == -1: break
                        cur = b
                        # get title (with the right encoding)
                        name = s[a:b].replace('\\\\u', '\\u').encode('utf-8').decode('unicode-escape').encode('latin1').decode('utf-8')
                        if not name.startswith('[GBF]'): continue # only take videos starting with [GBF]
                        c = name.find(separator) # cut at the separator position
                        if c != -1: name = name[:c]
                        name = name[6:]
                        out.append([code, name]) # add to search results
                        if len(out) == limit: break
                    if len(out) == 0:
                        final_msg = await ctx.send(embed=self.bot.buildEmbed(title="Error", description="No matches found", footer=query, color=self.color))
                    elif len(out) == 1:
                        final_msg = await ctx.send(embed=self.bot.buildEmbed(title=out[0][1], url='https://youtu.be/{}'.format(out[0][0]), image='https://i.ytimg.com/vi/{}/hqdefault.jpg'.format(out[0][0]), footer=query, color=self.color))
                    else:
                        msg = ""
                        for r in out:
                            msg += ":white_small_square: [{}](https://youtu.be/{})\n".format(r[1], r[0])
                        final_msg = await ctx.send(embed=self.bot.buildEmbed(title=("Search results" if url != url2 else "Latest videos"), description=msg, image='https://i.ytimg.com/vi/{}/hqdefault.jpg'.format(out[0][0]), footer=query, color=self.color))
                    await self.bot.cleanMessage(ctx, final_msg, 30)