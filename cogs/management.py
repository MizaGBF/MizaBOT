...
class Management(commands.Cog):
    ...
    async def status(self, ctx):
        """Post the bot status"""
        await ctx.send(embed=self.bot.buildEmbed(title="{}'s status".format(ctx.guild.me.display_name ), description="**Uptime**▫️{}\n**CPU**▫️{}%\n**Memory**▫️{}MB\n**Save Pending**▫️{}\n**Message Cache**▫️{}\n**Errors since boot**▫️{}\n**Tasks Count**▫️{}\n**Servers Count**▫️{}\n**Cogs Loaded**▫️{}/{}".format(self.bot.uptime(), self.bot.process.cpu_percent(), self.bot.process.memory_full_info().uss >> 20, self.bot.savePending, len(self.bot.cached_messages), self.bot.errn, len(asyncio.all_tasks()), len(self.bot.guilds), len(self.bot.cogs), self.bot.cogn), thumbnail=ctx.guild.me.avatar_url, color=self.color))