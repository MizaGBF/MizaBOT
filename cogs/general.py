...
class General(commands.Cog):
    ...
    @utility.sub_command()
    async def dropchance(self, inter, chance : str = commands.Param(description="Drop rate of the item (Format: Either XX% or 0.X)"), tries : int = commands.Param(description="Amount of tries, default is 1", default=1, ge=1, le=10000000)):
        ...
        try:
            if chance.endswith('%'):
                ...
                chance = float(chance)
            if chance >= 1:
                await inter.edit_original_message(embed=self.bot.util.embed(title="Drop Chance Calculator Error", description="Chance can't be higher or equal than 100% or 1", color=self.color))
            else:
    ...
    def get4chan(self, board : str, search : str): # be sure to not abuse it, you are not supposed to call the api more than once per second
        try:
            search = search.lower()
            data = self.bot.gbf.request('http://a.4cdn.org/{}/catalog.json'.format(board), expect_JSON=True, no_base_headers=True, follow_redirects=True)
            threads = []
            for p in data:
                for t in p["threads"]:
                    try:
