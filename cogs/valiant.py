...
class Valiant(commands.Cog):
    ...
    def getValiantState(self): # return the current state of the valiant in string format (which day is on going, etc...)
        if self.bot.valiant['state'] == True:
            ...
            if current_time < self.bot.valiant['dates']["Day 1"]:
                ...
                return "{} March of the Valiant starts in **{}**".format(self.bot.getEmote('time'), self.bot.getTimedeltaStr(d, True))
    ...
    @commands.command(no_pm=True, cooldown_after_parsing=True, aliases=['march'])
    @commands.cooldown(1, 10, commands.BucketType.guild)
    async def Valiant(self, ctx, gmt :int = 9):
        """Post the Valiant schedule"""
        if self.bot.valiant['state'] == True:
            try:
                ...
                if current_time < self.bot.valiant['dates']["End"]:
                    ...
                    for i in range(len(day_list)):
                        if day_list[i] == 'End': break
                            ...
                            description += "▫️ {:}: **{:%a. %m/%d %H:%M}**\n".format(day_list[i], self.bot.valiant['dates'][day_list[i]] + timedelta(seconds=3600*(gmt-9)))
                else:
                    await ctx.send(embed=self.bot.buildEmbed(title="{} **March of the Valiant**".format(self.bot.getEmote('gw')), description="Not available", color=self.color))
                    self.bot.valiant['state'] = False
                    self.bot.valiant['dates'] = {}
                    self.bot.savePending = True
                    return
                ...
                try:
                    description += self.getValiantState()
