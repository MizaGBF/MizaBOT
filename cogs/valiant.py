    ...
    async def fugdidbarragestart(self, ctx):
        ...
        try:
            ...
            if d != "":
                em = self.bot.getEmote(self.bot.valiant.get('element', ''))
                if em == "": em = ":white_small_square:"
                await ctx.reply(embed=self.bot.buildEmbed(title="{} **Dread Barrage {}** {} status".format(self.bot.getEmote('crew'), self.bot.valiant['id'], em), description=d, color=self.color))
        except Exception as e:
